    ...
    if (input.provProfilePath) {
        ...
        if (fs.existsSync(profilePath) && fs.lstatSync(profilePath).isFile()) {
            ...
            // Get UUID of provisioning profile
                 // on macOS Sierra, the security command has an issue and cannot get UUID properly
//              return exec('/usr/libexec/PlistBuddy -c "Print UUID" /dev/stdin <<< $(/usr/bin/security cms -D -i "' + profilePath + '")')
                 //use open ssl command instead
                 return exec('/usr/libexec/PlistBuddy -c "Print UUID" /dev/stdin <<< $(openssl smime -inform der -verify -noverify -in "' + profilePath + '")')
...
function removeExecOutputNoise(input) {
    ...
    // openssl command may output the string 'Verification successful', so also consider it as noise 
